...
@section('content')
    ...
    <div id="index_page" class="d-none">
                    ...
                    <defs>
                        ...
                        <g id="cat">
                            ...
                            <circle fill="#FFF" stroke="#0170BB" stroke-width="3" stroke-linecap="round"
                                stroke-linejoin="round" stroke-miterlimit="10" cx="1118.6" cy="621.7" r="23.1" />
        ...
        {{-- <div class="form-group p-4">
            <div class="org-selector-wrapper">
                ...
                <label class="w-50">
                    <input type="radio" class="org-radio-input member_status" name="member_status" id="member"
                        value="member">
                    <div class="org-card">
    ...
    <div class="container-fluid p-0" style="max-width: 600px; margin: 0 auto;">
        ...
        <div class="material-card mb-4 pb-4">
            ...
            <div id="have-info-not-line-id" class="d-none">
                 <div class="px-4">
                    <div class="form-floating mb-3">
                                    <input type="tel" name="phone" id="phone" class="form-control border-danger"
                                        placeholder="เบอร์โทร" value="0956650093">
                                    <label>หมายเลขโทรศัพท์ผู้สมัคร</label>
                    </div>
                     <button type="button" id="phone_search_btn2"
            ...
            <div id="new_user_form" class="d-none">
                    <form id="registerForm">
                        ...
                        <div class="form-group mb-4">
                            ...
                            <div class="org-selector-wrapper">
                                <label class="w-50">
                                    <input type="radio" class="org-radio-input" name="org_type_selector" id="type_general"
                                        value="general">
                                    <div class="org-card">
                                ...
                                <label class="w-50">
                                    <input type="radio" class="org-radio-input" name="org_type_selector" id="type_hospital"
                                        value="hospital">
                                    <div class="org-card">
                            ...
                            <div class="row g-2">
                                <div class="col-4">
                                    <input type="text" id="show_province"
                                        class="form-control form-control-sm bg-white border-0" disabled placeholder="จ.">
                                </div>
                                <div class="col-4">
                                    <input type="text" id="show_district"
                                        class="form-control form-control-sm bg-white border-0" disabled placeholder="อ.">
                                </div>
                                <div class="col-4">
                                    <input type="text" id="show_tambon"
                                        class="form-control form-control-sm bg-white border-0" disabled placeholder="ต.">
                                </div>
                            </div>
                        </div>
...
@section('script')
    ...
    <script>
        ...
        const checkPermission = async () => {
            try {
                // 1. ตรวจสอบสถานะสิทธิ์ profile
                const permissionStatus = await liff.permission.query('profile');

                if (permissionStatus.state === 'denied') {
                    // ถ้าเคยปฏิเสธไปแล้ว ให้ลองขอใหม่ (จะเด้งหน้าจอ Consent เขียวๆ)
                    await liff.permission.request(['profile']);
                    // เมื่อขอเสร็จ ให้ตรวจสอบอีกครั้ง
                    const retryStatus = await liff.permission.query('profile');
                    return retryStatus.state === 'granted';
                } else if (permissionStatus.state === 'prompt') {
                    // ถ้ายังไม่เคยขอเลย ให้ขอสิทธิ์
                    await liff.permission.request(['profile']);
                    return true;
                }
        ...
        const main = async () => {
            try {
                ...
                // --- เพิ่มการตรวจสอบและขอสิทธิ์ที่นี่ ---
                const isAuthorized = await checkPermission();

                if (!isAuthorized) {
                    alert("กรุณาอนุญาตการเข้าถึงข้อมูลโปรไฟล์เพื่อให้ระบบทำงานได้ถูกต้อง");
                    // คุณอาจจะเลือก liff.closeWindow() หรือปล่อยให้เขากรอกเองก็ได้
                }

                // ดึงข้อมูล Profile (หลังจากตรวจสอบสิทธิ์แล้ว)
                    ...
                    $.post(`/api/line/find_line_id`, {
                        ...
                        if (data.res == 0) {
                          ...
                          //  $('#new_user_form').removeClass('d-none')
                            $('#have-info-not-line-id').removeClass('d-none')
        ...
        $('#phone_search_btn2').on('click', async function () {
            let firstname = $('#firstname').val();
            let lastname = $('#lastname').val();
            let _phone = phone_text.value;
            let res = true;

            // หา user จาก phone
             $.post(`/api/line/findUserByPhone`,{phone:_phone}).done(function (data) {
                console.log('xx',data)
                const user_info = data.user_info
               if(Object.keys(user_info).length > 0){
                   
                    if(user_info.haved_line_id === 1){
                        //  const response = await $.post(`/line/update_user_line_id`,  {line_user_id: profile.userId });
                        //     console.log("response data:", response);

                        window.location.href = `/line/dashboard/${user_info.user_id}/${user_info.org_id}/1`;
                    }
               }else{
    ...
    <script>
        ...
        $('input[name="org_type_selector"]').change(function () {
            let type = $(this).val();
            let org_label = 'ระบุชื่อเทศบาล/อบต.'
            if (type === 'uni') {
                org_label = 'ระบุชื่อมหาวิทยาลัย';
        ...
        function filterOrgList(type) {
            console.log('ty2', type)
            orgListCache = []; // Reset Cache
            // Loop ข้อมูลดิบ แล้วเลือกเฉพาะที่ตรงประเภท
            for (const [key, org] of Object.entries(allOrganizations)) {

                console.log('org',org)
                let isUni = (org.orgType.code === 'ม.');
                let isLgov = (org.orgType.code === 'อบต.' || org.orgType.code === 'ทต.');
                let isHospital = (org.orgType.code === 'รพ.' || org.orgType.code === 'รพสต.');
        ...
        function handleOrgSelection(orgData, id) {
            // 1. Set ค่าพื้นฐาน
            console.log('orgData.org_type_name', orgData.org_type_name)
