...
@section('style')
    <style>
        ...
        .card-title {
            ...
            color: #344767;
            display: flex;
            /* Make title and toggle icon inline */
            align-items: center;
            /* Vertically align items */
        }

        .btn-link {
            /* Style for the toggle button/link */
            font-size: 0.9rem;
            font-weight: 600;
            color: #344767;
...
@section('content')
    <a href="{{ route('keptkayas.admin.kp_user.create') }}" class="btn bg-gradient-success mb-3">เพิ่มผู้ใช้งานระบบ</a>
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h3 class="card-title mb-0">
                    ค้นหาจากหมู่บ้าน
                    <a class="btn btn-link text-dark p-0 ms-2" data-bs-toggle="collapse" href="#collapseSearchFilters"
                        role="button" aria-expanded="false" aria-controls="collapseSearchFilters">
                                ...
                                <div class="form-check me-2">
                                    @if (isset($subzone_search_lists))
                                        <input class="form-check-input" type="checkbox" name="subzone_id_lists[]"
                                            value="{{ $subzone->id }}" {{ in_array($subzone->id, $subzone_search_lists) == true ? 'checked' : '' }}>
                                    @else
                                            <input class="form-check-input" type="checkbox" name="subzone_id_lists[]"
                                                value="{{ $subzone->id }}">
                                        </div>
    ...
    <form action="{{ route('keptkayas.admin.kp_user.store') }}" method="POST">
        @csrf
        @if ($checkBinsEmpty > 0)
                    ...
                    <tr>
                        ...
                        <td style="width: 20%">
                            @if ($bin_count == 0)
                                ...
                                <input type="text" class="form-control" name="kp_u_info[{{ $i }}][bins]" value="{{ $bin_count }}">
                            @else
                                    ...
                                    <div class="card-header p-2 d-flex justify-content-between align-items-center">
                                        ...
                                        <div class="card-tools">
                                            ...
                                            <button type="button" class="btn btn-tool" data-bs-toggle="collapse"
                                                ...
                                                <i class="fas fa-chevron-down toggle-bin-icon"
                                                    id="toggleBinIcon-{{ $userKeptKayaInfo->kp_u_infos_id }}"></i>
                                            </button>
                                        </div>
                                    </div>
                        ...
                        <td class="project-actions text-right">
                            ...
                            <a class="btn btn-info btn-sm"
                                href="{{ route('keptkayas.admin.kp_user.edit', $userKeptKayaInfo) }}">
                                <i class="fas fa-user"></i> แก้ไขมูล
                            </a>
                            <a class="btn btn-warning btn-sm"
                                href="{{ route('keptkayas.user-monthly-status.manage', $userKeptKayaInfo) }}">
