        ...
        <div class="col-lg-8 col-md-10 mx-auto">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title mb-0">แก้ไขอัตราค่าบริการ: <span
                            class="text-primary">{{ $payrate->kpUsergroup->name ?? 'N/A' }} ปี
                            {{ $payrate->budgetYear->year ?? 'N/A' }}</span></h3>
                </div>
                <div class="card-body">
                    @if ($errors->any())
                    ...
                    <form action="{{ route('keptkayas.payrate_per_months.update', $payrate->id) }}" method="POST">
                        @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="kp_usergroup_idfk" name="kp_usergroup_idfk" required>
                                ...
                                @foreach ($usergroups as $group)
                                    <option value="{{ $group->id }}" {{ (old('kp_usergroup_idfk', $payrate->kp_usergroup_idfk) == $group->id) ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            <label for="payrate_peryear" class="form-label">อัตราค่าบริการรายปี <span
                                    class="text-danger">*</span></label>
                            <input type="number" step="0.01" class="form-control" id="payrate_peryear"
                                name="payrate_peryear" value="{{ old('payrate_peryear', $payrate->payrate_peryear) }}"
                                required placeholder="เช่น 1200.00">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="status" name="status" required>
                                <option value="active" {{ (old('status', $payrate->status) == 'active') ? 'selected' : '' }}>
                                    Active</option>
