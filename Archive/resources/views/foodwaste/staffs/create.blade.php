                        ...
                        <div class="mb-3">
                            ...
                            <div class="row">
                                @foreach($roles as $role)
                                    {{-- @if(in_array($role->name, ['access waste bank', 'access annual collection'])) Only show relevant roles --}}
                                        <div class="col-md-6 col-lg-4">
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox"
                                                       id="perm_{{ Str::slug($role->name) }}"
                                                       name="roles[]"
                                                       value="{{ $role->name }}"
                                                       {{ in_array($role->name, old('roles', [])) ? 'checked' : '' }}>
                                                <label class="form-check-label" for="perm_{{ Str::slug($role->name) }}">
                                                    {{ $role->name  }}
                                                </label>
                                            </div>
                                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="row">
                                @foreach($permissions as $permission)
                                    {{-- @if(in_array($permission->name, ['access waste bank', 'access annual collection']))  --}}
