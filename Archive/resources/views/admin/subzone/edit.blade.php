...
@section('style')
    เพิ่มเส้นทางจดมิเตอร์ พื้นที่ {{$zone->zone_name}}
@endsection
@section('nav')
...
@section('content')
    เพิ่มเส้นทาง พื้นที่ {{$zone->zone_name}}
    <div class="card">
        <div class="card-body">
            <form action="{{route('admin.super_admin.subzone.update', $zone->id)}}" method="POST"
                onsubmit="return checkZoneNameValues()">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-lg-8 mb-4">
                        <div class="card card-widget widget-user">
                            <div class="card-footer">
                                <div id="routList">
                                    <?php $i = 0; ?>
                                    @foreach ($zone->subzone as $item)
                                        @if (collect($zone->subzone)->count() == $i)
                                            <div class="input-group mb-3 route ">
                                                <input type="text" class="form-control subzone_name" data-id="{{$i}}"
                                                    name="subzone[{{$i}}][{{$item['id']}}][subzone_name]" id="routeNameTemp"
                                                    value="{{$item['subzone_name']}}">
                                                <a href="javascript:void(0)" class="btn btn-primary permanentVal addRoute "
                                                    data-subzone_id="{{$item['id']}}" data-i="{{$i}}">เพิ่ม</a>
                                            </div>
                                        @else
                                            <div class="input-group mb-3 route last">
                                                <input type="text" class="form-control"
                                                    name="subzone[{{$i}}][{{$item['id']}}][subzone_name]"
                                                    id="routeNameTemp{{$item['id']}}" value="{{$item['subzone_name']}}">
                                                <a href="javascript:void(0)" data-subzone_id="{{$item['id']}}" data-i="{{$i}}"
                                                    id="a_del{{$i}}" class="btn btn-danger permanentVal removeRoute">ลบ</a>
                                            </div>
                                        @endif
                                        <?php    $i++; ?>
                                    @endforeach
                                    <div class="input-group mb-3 route last">
                                        <input type="text" class="form-control subzone_name" data-id="{{$i}}"
                                            name="subzone[{{$i}}][new][subzone_name]" id="routeNameTemp" value="">
                                        <a href="javascript:void(0)" class="btn btn-primary permanentVal addRoute last"
                                            data-subzone_id="new" id="a_del{{$i}}" data-i="{{$i}}">เพิ่ม</a>
                                    </div>
                                </div>
