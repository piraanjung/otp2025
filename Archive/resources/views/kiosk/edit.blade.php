...
@section('content')
    <div class="container mt-4">
        <div class="card shadow-sm" style="max-width: 600px; margin: auto;">
            ...
            <div class="card-body">
                <form action="{{ route('keptkayas.kiosks.update', $kiosk->id) }}" method="POST">
                    @csrf
                    <div class="mb-3">
                        <label>Kiosk ID (Hardware ID)</label>
                        <input type="text" name="id" class="form-control" value="{{ old('id', $kiosk->id ?? '') }}" placeholder="เช่น SLAVE_01" required>
                        <small class="text-muted">ต้องตรงกับ Code ใน ESP8266</small>
                    </div>

                    <div class="mb-3">
                        <label>ชื่อจุดตั้ง/อาคาร</label>
                        <input type="text" name="name" class="form-control"  value="{{ old('name', $kiosk->name ?? '') }}" placeholder="เช่น โรงอาหารกลาง" required>
                    </div>
