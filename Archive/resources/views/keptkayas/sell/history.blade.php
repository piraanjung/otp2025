...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route('keptkayas.sell.form') }}" class="btn btn-primary">
                    ...
                    <tbody>
                        @forelse ($transactions as $transaction)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('keptkayas.sell.receipt', $transaction->id) }}"
                                        class="btn btn-info btn-sm">
                                        <i class="fa fa-receipt me-1"></i> ใบเสร็จ
                                    </a>
                                    <form action="{{ route('keptkayas.sell.destroy', $transaction->id) }}" method="POST"
                                        class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger btn-sm"
                                            onclick="return confirm('คุณแน่ใจหรือไม่ที่จะลบธุรกรรมนี้?')"><i
