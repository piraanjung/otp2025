...
class KorKor3Export implements FromQuery, WithHeadings, WithMapping, ShouldAutoSize, WithStyles
{
    ...
    public function query()
    {
        ...
        $query = AnnualTrashSubscription::query()
            ->with(['AnnualTrash.user', 'AnnualTrash.kpUserGroup'])
            ->where('fiscal_year', $fy); // ตอนนี้ $fy จะไม่เป็น null แล้ว

        // --- ส่วนการค้นหา (Search) ---
        if ($search) {
            $query->whereHas('AnnualTrash', function ($q) use ($search) {
    ...
    public function map($row): array
    {
        ...
        return [
            $row->AnnualTrash->bin_code, // รหัสถัง
            ($row->AnnualTrash->user->firstname ?? '-') . ' ' . ($row->AnnualTrash->user->lastname ?? ''), // ชื่อ-สกุล
            $row->AnnualTrash->user->address ?? '-', // ที่อยู่
            $row->AnnualTrash->kpUserGroup->usergroup_name ?? 'ทั่วไป', // ประเภท
