...
class WasteBinPayratePerMonthController extends Controller
{
    ...
    public function index()
    {
        ...
        $payrates = WasteBinPayratePerMonth::with(['kp_usergroup', 'budgetyear'])
            ...
            ->paginate(10);
        return view('keptkayas.wbin_payrate_per_months.index', compact('payrates'));
    }
    ...
    public function create()
    {
        ...
        return view('keptkayas.wbin_payrate_per_months.create', compact('usergroups', 'budgetYears'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $existingRecord = WasteBinPayratePerMonth::where('kp_usergroup_idfk', $request->kp_usergroup_idfk)
            ...
            ->first();
        if ($existingRecord) {
            return redirect()->route('keptkayas.wbin_payrate_per_months.index')
                ->with('error', 'xx');
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $payrate->update([ /* ... data ... */]);
            return redirect()->route('keptkayas.wbin_payrate_per_months.index')->with('success', 'อัปเดตอัตราค่าบริการสำเร็จแล้ว.');
