...
@section('style')
    ...
    <style>
        ...
        .table tbody td {
            ...
            text-align: center; /* จัดให้อยู่ตรงกลางตามต้องการ */
        }
        .table tbody td:nth-child(2), /* กลุ่มผู้ใช้ */
        .table tbody td:nth-child(4) /* อัตราค่าบริการ */
        {
            text-align: left; /* จัดชิดซ้ายสำหรับข้อความ */
        }
        .badge {
            font-size: 0.75rem;
            padding: 0.4em 0.7em;
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <div class="card-tools">
                    <a href="{{ route('keptkaya.payrate_per_months.create') }}" class="btn btn-primary btn-sm">
                    ...
                    <table class="table table-hover align-items-center mb-0" id="payratesTable">
                        <thead>
                            <tr>
                                ...
                                <th class="text-uppercase text-secondary text-center font-weight-bolder ps-2">กลุ่มผู้ใช้</th>
                                <th class="text-uppercase text-secondary text-center font-weight-bolder">ปีงบประมาณ</th>
                                <th class="text-uppercase text-secondary text-center font-weight-bolder">อัตราค่าบริการต่อเดือน</th>
                                <th class="text-uppercase text-secondary text-center font-weight-bolder">อัตราค่าบริการรายปี</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><p class="text-xs font-weight-bold mb-0">{{ $payrate->id }}</p></td>
                                ...
                                <td>
                                    <span class="badge badge-sm {{ $payrate->status == 'active' ? 'bg-gradient-success' : 'bg-gradient-danger' }}">
                                        {{ $payrate->status == 'active' ? 'ใช้งาน' : 'ไม่ใช้งาน' }}
                                    </span>
                                </td>
                                <td>
                                    <a href="{{ route('keptkaya.payrate_per_months.edit', $payrate->id) }}" class="btn btn-sm btn-warning mb-0 me-1">
                                        <i class="fas fa-edit"></i> แก้ไข
                                    </a>
                                    <form action="{{ route('keptkaya.payrate_per_months.destroy', $payrate->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-danger mb-0" onclick="return confirm('คุณแน่ใจหรือไม่ที่จะลบรายการนี้? การลบจะเป็นการปิดใช้งานข้อมูลนี้')">
