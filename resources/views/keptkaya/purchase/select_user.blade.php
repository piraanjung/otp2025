...
<div class="container">
    <div class="card">
        <div class="card-header bg-primary text-white">
            <h1 class="card-title mb-0">เลือกผู้ใช้งานสำหรับทำธุรกรรม</h1>
        </div>
        <div class="card-body">
            {{-- Filter/Search Form --}}
                    ...
                    <form action="{{ route('keptkaya.purchase.select_user') }}" method="GET" id="user-search-form">
                        <div class="row g-3">
                            <div class="col-md-5">
                                ...
                                <input type="text" name="name_search" id="name_search" class="form-control" value="{{ request('name_search') }}">
                            </div>
                            <div class="col-md-5">
                                <label for="username_search" class="form-label">Username:</label>
                                <input type="text" name="username_search" id="username_search" class="form-control" value="{{ request('username_search') }}">
                            </div>
            ...
            <div class="d-grid gap-2 mb-4">
                <button class="btn btn-success btn-lg" type="button" data-bs-toggle="modal" data-bs-target="#qrScannerModal">
                    <i class="bi bi-qr-code-scan me-2"></i> สแกน QR Code
                </button>
            </div>
                        ...
                        @forelse ($keptKayaMembers as $member)
                            <tr>
                                ...
                                <td>
                                    @if ($member->purchaseTransactions->count() > 0)
                                        <div class="p-2 mb-2 rounded bg-light">
                                            ...
                                            <ul class="list-unstyled mb-0 small">
                                                ...
                                                <li>
                                                    <strong>ยอดรวม:</strong> {{ number_format($todayTransactions->sum('total_amount'), 2) }} บาท
                                                </li>
                                                <li>
                                                    <li>
                                                 <a href="{{ route('keptkaya.purchase.receipt', $todayTransactions[0]->id) }}" class="btn btn-sm btn-info">
                                    ...
                                    <a href="{{ route('keptkaya.purchase.history', $member->id) }}" class="btn btn-info btn-sm me-2">
                                        <i class="bi bi-clock-history me-1"></i> ดูประวัติ
                                    </a>
                                    <a href="{{ route('keptkaya.purchase.start_purchase', $member->id) }}" class="btn btn-success btn-sm">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12">
                        <div class="card h-100 shadow-sm border-secondary">
                            <div class="card-body">
                                <h5 class="card-title"><strong>{{ $member->firstname }} {{ $member->lastname }}</strong></h5>
                                ...
                                <div class="d-flex justify-content-between mt-3">
                                    <a href="{{ route('keptkaya.purchase.history', $member->id) }}" class="btn btn-info btn-sm">
                                        <i class="bi bi-clock-history me-1"></i> ดูประวัติ
                                    </a>
                                    <a href="{{ route('keptkaya.purchase.start_purchase', $member->id) }}" class="btn btn-success btn-sm">
                                        <i class="bi bi-cart-plus me-1"></i> เริ่มรับซื้อ
