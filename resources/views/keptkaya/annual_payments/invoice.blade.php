...
<div class="container my-5">
    <div class="card shadow-lg">
        ...
        <div class="card-body">
            ...
            @else
                <form id="print-form" action="{{ route('keptkaya.annual_payments.print_selected_invoices') }}" method="POST">
                    @csrf
                            ...
                            <tbody>
                                @foreach ($invoices as $index => $invoice)
                                    <tr>
                                        ...
                                        <td>{{ $index + 1 }}</td>
                                        <td>{{ $invoice->wasteBin->user->firstname }} {{ $invoice->wasteBin->user->lastname }}</td>
                                        ...
                                        <td>
                                            <a href="{{ route('keptkaya.annual_payments.show', $invoice->id) }}" class="btn btn-primary btn-sm">
                                                <i class="bi bi-eye"></i> ดูรายละเอียด
                                            </a>
                                            {{-- Assuming a payment route for this item --}}
                                            {{-- <a href="{{ route('keptkaya.annual_payments.edit_payment', $invoice->id) }}" class="btn btn-success btn-sm">
        ...
        <div class="card-footer text-end">
            ...
            <a href="{{ route('keptkaya.annual_payments.index') }}" class="btn btn-secondary">
