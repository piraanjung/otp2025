...
<div class="container mx-auto p-4 max-w-lg">
    <h1 class="text-2xl font-semibold mb-6">Edit Machine: {{ $machine->machine_id }}</h1>
    
    <form action="{{ route('machines.update', $machine) }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        @csrf
        @method('PUT')

        {{-- Machine ID (อาจจะไม่ควรแก้ไขในความเป็นจริง) --}}
        <div class="mb-4">
            <label class="block text-gray-700 text-sm font-bold mb-2" for="machine_id">Machine ID</label>
            <input type="text" name="machine_id" id="machine_id" value="{{ old('machine_id', $machine->machine_id) }}" required 
        ...
        <div class="mb-4">
            ...
            <input type="number" name="org_id_fk" id="org_id_fk" value="{{ old('org_id_fk', $machine->org_id_fk) }}" required 
        ...
        <div class="mb-4">
            ...
            <select name="status" id="status" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                @foreach (['offline', 'idle', 'busy', 'online'] as $status)
                    <option value="{{ $status }}" {{ old('status', $machine->status) == $status ? 'selected' : '' }}>
                        {{ ucfirst($status) }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="pending_command" id="pending_command" value="{{ old('pending_command', $machine->pending_command) }}"
        ...
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('machines.index') }}" class="inline-block align-baseline font-bold text-sm text-blue-500 hover:text-blue-800">
                Cancel
            </a>
        </div>
    </form>
