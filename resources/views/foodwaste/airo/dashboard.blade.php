    ...
    <div class="container py-3 app-container">
        ...
        <div class="card shadow-sm mb-4 border-top border-success border-4">
            <div class="card-body p-4">
                ...
                @if($latestMeals->isNotEmpty())
                    <div class="mb-4 p-3 rounded-4 bg-light border-0" style="background-color: #f1f5f9 !important;">
                        ...
                        <div class="d-flex align-items-center">
                            <img src="{{ asset($latestEntry->photo_path) }}" class="rounded-3"
                ...
                <form id="store_waste_form" action="{{ route('foodwaste.store_waste') }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    ...
                    <input type="file" id="waste_photo" name="waste_photo" class="d-none" accept="image/*"
                        capture="camera" required onchange="previewImage(this, 'waste_preview', 'waste_placeholder')">

                    <div class="mb-4">
                        <label class="form-label-custom">น้ำหนักเศษอาหาร</label>
                        <div class="input-group">
                            <input type="number" step="10" name="weight_kg"
                                class="form-control form-control-lg border-0 bg-light rounded-start-pill ps-4"
                                placeholder="0" required>
                            <span
                                class="input-group-text border-0 bg-info text-bold rounded-end-pill pe-4 text-muted">กรัม</span>
                        </div>
                    </div>
                    ...
                    <div class="form-check mb-4 bg-light p-3 rounded-4">
                        <input class="form-check-input ms-1" type="checkbox" name="is_mixed" id="mixed"
                            style="transform: scale(1.2);">
                        <label class="form-check-label ms-3 small fw-bold text-success"
                            for="mixed">คลุกเคล้าขยะเรียบร้อยแล้ว</label>
                    </div>

                    <button type="submit"
                        class="btn btn-success w-100 rounded-pill py-3 shadow-sm fw-bold fs-5">บันทึกลงถังหมัก
                        🌿</button>
                </form>
            </div>
        </div>
