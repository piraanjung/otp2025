...
@section('content')
        ...
        <defs>
          ...
          <g id="plane">
            ...
            <text transform="matrix(1.0027 0 0 1 44.8218 224.8768)" font-family='Fredoka One' font-size="34"
              fill="#0170BB"> กรุณารอสักครู่.... </text>
            <path fill="#a5c7e4"
  ...
  <div class="card hidden" id="qrcode">
    <div class="card-header">
      <h4>สแกนกับเจ้าหน้าที่เพื่อทำการขายขยะ</h้>
...
@section('script')
  ...
  <script>
    ...
    const main = async () => {
      ...
      $.post(`/api/line/fine_line_id`, {
        ...
        if (data.res == 0) {
          ...
          $('#store_container').addClass('hidden')

        }
        if (data.res == 1) {
          makeQrcode(profile.userId)
        }
      })
    ...
    async function makeQrcode(line_user_id) {
      $('#store_container').addClass('hidden')
      $('#phone_div').addClass('hidden')
      
    await  new QRCode(document.getElementById("qrcode-container"), {
        text: line_user_id,
        width: 256,
        height: 256,
        colorDark: "#000000",
...
<script>
  ...
  const main = async () => {
    ...
    $.post(`/api/line/fine_line_id`, {
      ...
      if (data.res == 1) {
        // 1. Define the content you want to encode
