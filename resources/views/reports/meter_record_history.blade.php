        ...
        <form action="{{ route('reports.meter_record_history') }}" method="get">
            ...
            <div class="card-body row">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control js-example-tokenizer" name="budgetyear[]" id="budgetyear" data-placeholder="เลือก.."
                            ...
                            @foreach ($budgetyears as $budgetyear)
                                <option value="{{ $budgetyear->id }}" {{in_array($budgetyear->id, collect($budgetyear_selected_array)->toArray()) ? 'selected' : '' }}>{{ $budgetyear->budgetyear_name }}</option>
                            @endforeach
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control js-example-tokenizer" id="zone" name="zone[]" data-placeholder="เลือก.."
                            ...
                            @foreach ($zones as $zone)
                                <option value="{{$zone->id}}" {{ in_array($zone->id, $zone_id_array) ? 'selected' : '' }}>{{ $zone->zone_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="col-2">
                    ...
                    <button type="submit" name="submitBtn" value="search" class=" form-control  btn btn-success"> ค้นหา </button>
                    </div>
                </div>

                <div class="col-12 pt-2 row" style="border-top: 1px solid gray">
                    <button type="submit" name="submitBtn" value="export_excel" class="btn btn-info col-3"  > ดาวน์โหลดไฟล์ Excel  </button>
                </div>
