            ...
            <div class="col-md-8 mx-auto">
                <div class="card mb-4">
                    <div class="card-header pb-0">
                        <h6>แก้ไขข้อมูลเจ้าหน้าที่: {{ $staff->user->firstname ?? 'N/A' }}
                            {{ $staff->user->lastname ?? '' }}</h6>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('keptkayas.staffs.update', $staff->user_id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            @if(session('success'))
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="user_display"
                                    value="{{ $staff->user->prefix ?? '' }} {{ $staff->user->firstname ?? 'N/A' }} {{ $staff->user->lastname ?? '' }} ({{ $staff->user->email ?? 'N/A' }})"
                                    readonly>
                                            ...
                                            <div class="col-md-6 col-lg-4">
                                                <div class="form-check">
                                                    <input class="form-check-input" type="checkbox"
                                                        id="perm_{{ Str::slug($permission->name) }}" name="permissions[]"
